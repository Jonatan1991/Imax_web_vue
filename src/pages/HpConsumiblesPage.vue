    ...
    <v-container class="mt-8">
      ...
      <v-row class="mb-6 align-center">
        <v-col cols="12" md="6">
          <v-text-field
            ...
            density="comfortable"
            hide-details
            label="Buscar por modelo o impresora compatible..."
        ...
        <v-col class="text-md-right" cols="12" md="6">
          ...
          >
            ...
            <v-btn value="Tóner">Tóner</v-btn>
            <v-btn value="Tinta">Tinta</v-btn>
          </v-btn-toggle>
        </v-col>
      </v-row>
      ...
      <v-row v-else class="justify-center py-12">
        <v-col class="text-center" cols="12" md="6">
          ...
          <h3 class="text-h5 font-weight-bold text-grey-darken-2 mb-2">No se encontraron consumibles HP</h3>
...
<script setup lang="ts">
  ...
  onMounted(() => {
    document.title = 'HP Consumibles | Imax PC'
  })

  const filteredProducts = computed(() => {
    return offersStore.hpConsumibles.filter(product => {
...
.divider {
  width: 60px;
  height: 4px;
  background-color: #007DBA; /* HP Corporate Blue */
  border-radius: 2px;
}
